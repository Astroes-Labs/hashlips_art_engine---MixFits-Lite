...
const createDna = (_layers) => {
    ...
    // Apply rules based on selected Body
    rules.conditions.forEach(condition => {
        ...
        if (ifTraits.includes(bodyTrait)) {
            ...
            if (then.allowedLayers?.length > 0) rules.currentAllowed = then.allowedLayers;
            if (then.ignore?.length > 0) then.ignore.forEach(l => disabledLayers.add(l));
            if (then.forcedTraits) forcedTraits = { ...forcedTraits, ...then.forcedTraits };
            if (then.mutualExclusion) rules.currentMutualExclusion = then.mutualExclusion;
        }
    });
    ...
    _layers.forEach((layer, index) => {
        ...
        if (rules.ignoreTraits?.[layer.name]) {
            const toIgnore = Array.isArray(rules.ignoreTraits[layer.name])
                ? rules.ignoreTraits[layer.name]
                : [rules.ignoreTraits[layer.name]];
            availableElements = availableElements.filter(el => !toIgnore.includes(el.name));
        }

        let forcedTrait = forcedTraits[layer.name];
        if (forcedTrait) {
            ...
            if (forcedElement) {
                ...
                selectedTraits[layer.name] = forcedTrait;
                applyMutualExclusions(layer.name, forcedTrait, disabledLayers);
                return;
            }
        }
        ...
        for (let el of availableElements) {
            ...
            if (random < 0) {
                ...
                selectedTraits[layer.name] = el.name;
                applyMutualExclusions(layer.name, el.name, disabledLayers);
                break;
            }
        }
    });
